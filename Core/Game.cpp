...
void CGame::CreateShaders()
{
	...
	PSNoise = make_unique<CShader>(m_Device.Get(), m_DeviceContext.Get());
	PSNoise->Create(EShaderType::PixelShader, L"Shader\\PSNoise.hlsl", "main");
	PSNoise->AddConstantBuffer(&cbPSNoiseData, sizeof(SCBPSNoiseData));
}
...
void CGame::DrawGameObject2Ds(float DeltaTime)
{
	...
	for (auto& i : m_vGameObject2Ds)
	{
		...
		if (GO2D->ComponentRender.PtrObject2D)
		{
			...
			{
				if (GO2D->bUseNoiseTexture)
				{
					PSNoise->Use();
					cbPSNoiseData.Time = TimeFlow;
					PSNoise->UpdateConstantBuffer(0);
				}
				else
				{
					PSBase2D->Use();
