...
int WINAPI WinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPSTR lpCmdLine, _In_ int nShowCmd)
{
	...
	CTexture* TextureNoise{ Game.AddTexture() };
	{
		TextureNoise->CreateNoiseTexture(XMFLOAT2(200, 200));
	}
	...
	CGameObject2D* go2DNoise{ Game.AddGameObject2D("Noise") };
	{
		go2DNoise->ComponentRender.PtrObject2D = Game.AddObject2D();
		go2DNoise->ComponentRender.PtrObject2D->CreateDynamic(Generate2DRectangle(XMFLOAT2(500, 500)));
		go2DNoise->ComponentRender.PtrTexture = TextureNoise;
		go2DNoise->bUseNoiseTexture = true;
	}
